...
def parse(lines):
    ...
    header_pat = re.compile(r"\[T\]([A-Z]{2}\d{3})\s+([1-5])\s+2\s+([135])")
    ...
    while i < len(lines):
        ...
        if "[T]" in line:
            ...
            if not answer or not re.fullmatch(r"[A-D]{2,}", answer):
                logging.error(f"多选题 编码 {code} {level_code} 2 {difficulty_code} 答案格式错误: {answer or ''}")
                answer = None
